...
from arxivscribe.ai_config import ai_config
...
class ArxivCommands(commands.Cog):
    ...
    config_group = app_commands.Group(name="config", description="Configure AI settings")

    @config_group.command(name="status", description="Show current AI configuration")
    async def config_status(self, interaction: discord.Interaction):
        """Show current LLM provider, model, and key status."""
        embed = discord.Embed(
            title="⚙️ AI Configuration",
            description=ai_config.get_status_text(),
            color=discord.Color.blue()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @config_group.command(name="set_key", description="Set your LLM API key (sent via DM for safety)")
    async def config_set_key(self, interaction: discord.Interaction, key: str):
        """Set the API key. Always ephemeral so the key isn't visible to others."""
        result = ai_config.update(api_key=key)
        # Also update the bot's summarizer
        try:
            self.bot.summarizer.set_provider(
                provider=ai_config.get_provider(),
                api_key=ai_config.get_api_key(),
                model=ai_config.get_model()
            )
        except Exception as e:
            logger.warning(f"Failed to hot-reload summarizer: {e}")
        await interaction.response.send_message(
            f"{result}\n\n**Current config:**\n{ai_config.get_status_text()}",
            ephemeral=True  # Only visible to the user who ran the command
        )

    @config_group.command(name="set_provider", description="Switch LLM provider (openai/huggingface)")
    async def config_set_provider(self, interaction: discord.Interaction, provider: str):
        """Switch LLM provider."""
        result = ai_config.update(provider=provider)
        try:
            self.bot.summarizer.set_provider(
                provider=ai_config.get_provider(),
                api_key=ai_config.get_api_key(),
    ...
    @config_group.command(name="set_model", description="Set the LLM model")
    async def config_set_model(self, interaction: discord.Interaction, model: str):
        """Set the model name."""
        result = ai_config.update(model=model)
        try:
            self.bot.summarizer.set_provider(
                provider=ai_config.get_provider(),
                api_key=ai_config.get_api_key(),
    ...
    @config_group.command(name="test", description="Test the current API connection")
    async def config_test(self, interaction: discord.Interaction):
        """Test the API connection."""
        await interaction.response.defer(ephemeral=True)
        result = await ai_config.test_connection()
        await interaction.followup.send(result, ephemeral=True)

    @config_group.command(name="clear_key", description="Clear the API key")
    async def config_clear_key(self, interaction: discord.Interaction):
        """Clear the stored API key."""
        ai_config.update(api_key="")
        await interaction.response.send_message(
            "✅ API key cleared.\n\n" + ai_config.get_status_text(),
            ephemeral=True
        )
